...
Runs on a 15-minute schedule via GitHub Actions.
Processes ALL pending Telegram updates on each run — no time window filter.
Telegram's offset system ensures each message is processed exactly once.
...
def _base_command(text: str) -> str:
    """Extract bare command, stripping @botname suffix Telegram adds in groups.
    '/fullscan@YourBotName' -> '/fullscan'
    """
...
def _parse_symbols(text: str) -> list[str]:
    """Strip the command word and return upper-cased symbol list."""
    parts = text.split()
    if parts and parts[0].startswith("/"):
        parts = parts[1:]   # drop the command word (already handled @botname above)
...
def _run_scan(symbols: list[str]) -> str:
    ...
    for meta in passed_s1:
        try:
            ...
            result = compute_buy_signal(
                df, daily, meta.symbol, meta.exchange, meta.company_name
            )
...
def _trigger_full_scan() -> str:
    """Dispatch the full-scan GitHub Actions workflow and return a status message."""
    token = os.environ.get("GITHUB_TOKEN", "")
    repo  = os.environ.get("GITHUB_REPOSITORY", "2yclocal/2yc-4h")

    if not token:
        return "⚠️ GITHUB_TOKEN not available — cannot trigger scan remotely."

    url = (
        f"https://api.github.com/repos/{repo}"
        f"/actions/workflows/full-scan.yml/dispatches"
    )
    resp = requests.post(
        url,
        headers={
...
def _trigger_watchlist_backtest() -> str:
    ...
    if not token:
        return "⚠️ GITHUB_TOKEN not available — cannot trigger backtest remotely."

    url = (
        f"https://api.github.com/repos/{repo}"
        f"/actions/workflows/watchlist-backtest.yml/dispatches"
    )
    resp = requests.post(
        url,
        headers={
...
def _handle(msg: dict):
    ...
    if cmd in ("/fullscan", "/runscan", "/daily"):
        ...
        _send(chat_id, f"📨 Command received at {picked_up} (sent {sent_at})\n\n{result}")
        return
    ...
    if cmd in ("/check", "/scan", "/filter"):
        ...
        _send(chat_id,
            f"📨 Command received at {picked_up} (sent {sent_at})\n"
...
def main():
    # Fetch all pending updates — no time filter.
    # Telegram's offset system guarantees each update is delivered exactly once.
    ...
    for update in updates:
        ...
        logger.info(
            ...
            f"\"{msg.get('text', '')}\" from {msg_time.strftime('%H:%M')} UTC"
        )
        try:
            _handle(msg)
            processed += 1
    ...
    # Advance the offset so processed updates are never re-delivered
    if last_id is not None:
        _api("getUpdates", offset=last_id + 1, timeout=0)
        logger.info(f"Acknowledged updates up to {last_id}")

    logger.info(f"Done — processed {processed} of {len(updates)} update(s)")
